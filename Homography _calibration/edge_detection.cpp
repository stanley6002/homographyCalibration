...
//
//  Created by chih-hsiang chang on 1/29/14.
//  Copyright 2014 __MyCompanyName__. All rights reserved.
//

#include <cstdio>
//# define Canny_thd 600
//# define Canny_mask 5
//# define BLACK 0
//# define WHITE 255
//# define line_threshold 25
//# define minLength  100
//# define tolerance 4.0   //  line tolerance
//# define segmentation 150
...
std::vector< std::vector <CvPoint> >edge_detection_test(IplImage* Iplimage, IplImage* Edge_map)
{
    
    //Edge_map=cvCreateImage(cvSize(Iplimage->width, Iplimage->height),IPL_DEPTH_8U,1)
    cvCanny(Iplimage,Edge_map,Canny_thd,2*Canny_thd,Canny_mask);
    //cvNamedWindow("AFTER_CANNY", CV_WINDOW_NORMAL);
    //cvResizeWindow("AFTER_CANNY", 400, 400 );
    //cvShowImage("AFTER_CANNY", Edge_map);
    eraseBorder(Edge_map);
    eliminateJunction(Edge_map, WHITE, BLACK);
    Image<uint8> iplEdgeT(Edge_map);
    IplImage* iplEdgeCpy = cvCloneImage(Edge_map);
    std::vector< std::vector <CvPoint> > vComp;
    
    for (int x = 1; x < Edge_map->width-1; x++)
    {
        for (int y = 1; y < Edge_map->height-1; y++)
        {
            uint8 s = iplEdgeT[y][x]; //cvGet2D(iplEdge,p.y,p.x);
            if (s!=WHITE) continue;   //already included in a list
            std::vector<CvPoint> vPt = floodfill(cvPoint(x,y), WHITE,  BLACK, iplEdgeCpy, line_threshold);
    ...
    //cvResizeWindow("example", 400, 400 );
    //cvShowImage("example", Edge_map);
